...
Django приложение для получения актуальной информации о странах.
...
1. Скопируйте файл настроек `.env.sample`, создав файл `.env`:
    ...
    * `API_KEY_NEWSAPI` - для тоекта доступа к NewsAPI
...
3. Чтобы просмотреть документацию по использованию консольного приложения, выполните:
    .. code-block:: console
        ...
        docker compose run countries-informer-app python manage.py --help
...
4. Для запуска приложения выполните:
    ...
    Для получения данных можно использовать следующие запросы:

    * http://localhost:8020/api/v1/city - получение городов по alpha-2 коду страны и названию города (указать в параметрах запроса)
    * http://localhost:8020/api/v1/city/{name} - получение городов по названию города
    * http://localhost:8020/api/v1/country - получение стран по alpha-2 коду (указать в параметрах запроса)
    * http://localhost:8020/api/v1/country/{name} - получение стран по названию
    * http://localhost:8020/api/v1/currency/{currency} - получение курсов валют для валюты по ISO 4217 буквенному коду валюты
    * http://localhost:8020/api/v1/weather/{alpha2code}/{city} - получение погоды по aplha-2 коду страны и названию города
    * http://localhost:8020/api/v1/news/{aplhpa2code} - получение новостей по aplha-2 коду страны

    Чтобы начать работу с админ панелью после запуска приложения необходимо создать пользователя, выполнив следующую команду:

    .. code-block:: console

        docker compose exec countries-informer-app python manage.py createsuperuser

    После создания пользователя можно открыть админ панель по следующему адресу: http://localhost:8020/admin/
...
.. code-block:: console
    ...
    docker compose run countries-informer-app pytest --cov=/src --cov-report html:htmlcov --cov-report term --cov-config=/src/tests/.coveragerc -vv
